...
class NFB_Frontend
{
    ...
    /**
     * افزودن فایل‌های CSS و JS به سایت.
     */
    public function enqueue_scripts()
    {
        wp_enqueue_style('nfb-frontend-style', NFB_PLUGIN_URL . 'assets/css/frontend.css', [], NFB_VERSION);
        wp_enqueue_script('nfb-frontend-script', NFB_PLUGIN_URL . 'assets/js/frontend.js', ['jquery'], NFB_VERSION, true);

        // کتابخانه امضای دیجیتال را فقط در صورت نیاز بارگذاری می‌کنیم
        // این کار باید به صورت هوشمند بر اساس فیلدهای فرم انجام شود.
        // در حال حاضر برای سادگی، همیشه بارگذاری می‌شود.
    ...
    /**
     * رندر کردن فرم با استفاده از شورت‌کد.
     *
    ...
    private function render_form_html($form_id)
    {
        ...
        // بررسی محدودیت‌های ارسال فرم
        $restrictions_error = self::validate_submission_restrictions($form_id, $settings['restrictions'] ?? []);
        if ($restrictions_error) {
            echo '<div class="nfb-alert nfb-alert-warning">' . esc_html($restrictions_error) . '</div>';
            return;
        }
        ...
        if (!empty($fields) && is_array($fields)) {
            ...
            if ($has_steps) {
                ...
                echo '<div class="nfb-step active">'; // Start first step
            }

            NFB_Core::render_fields_recursive($fields, 0, [], 'nfb_fields');

            if ($has_steps) {
                echo '</div>'; // End last step
                echo '</div>'; // End steps container
    ...
    private function render_success_message($form_id)
    {
        ...
        // **بخش جدید: افزودن دکمه دانلود PDF**
        $pdf_url = isset($_GET['pdf_url']) ? esc_url_raw($_GET['pdf_url']) : '';
    ...
    public function handle_form_submission()
    {
        ...
        // اعتبارسنجی سمت سرور
        // ... (منطق اعتبارسنجی در اینجا پیاده‌سازی می‌شود)
        ...
        if ($actions_result['status'] === 'success') {
            ...
            $redirect_url = add_query_arg([
                'nfb_status' => 'success',
                'nfb_form_id' => $form_id,
                'pdf_url' => urlencode($actions_result['pdf_url'] ?? '') // **ارسال URL به پیام موفقیت**
            ], $redirect_url);
        } else {
            // در صورت خطا، ورودی را حذف می‌کنیم تا دیتابیس شلوغ نشود
    ...
    /**
     * اعتبارسنجی محدودیت‌های ارسال.
     */
    private static function validate_submission_restrictions($form_id, $restrictions)
    {
